...
# STEP 2: Load Zheng .h5 files (excluding mouse bone marrow)
message("\nLoading Zheng .h5 files...")
h5_files <- list.files(path = "Zheng", pattern = "\\.h5$", full.names = TRUE)
h5_files <- h5_files[!grepl("mBM", basename(h5_files))]  # Exclude mouse bone marrow
message(paste("Found", length(h5_files), ".h5 files (after excluding mouse BM)"))
...
bm_samples <- bm_samples[!grepl("^mBM", bm_samples)]  # Exclude mouse bone marrow
...
# STEP 8: Set developmental order for x-axis
message("\n=== SETTING DEVELOPMENTAL ORDER ===")

# Define developmental order (least to most developed)
developmental_order <- c(
  # 1. AGM - Nascent HSCs (4-5 weeks PCW)
  "agm-4wk-658",
  "agm-5wk-555",
  "agm-5wk-575",
  
  # 2. Fetal Liver - Early (6 weeks PCW / CS16)
  "FL_CS16_W9",
  
  # 3. Fetal Liver - Mid (8 weeks PCW / CS22)
  "FL1_hpc_CS22",
  "FL2_hpc_CS22",
  
  # 4. Bone Marrow - Early to Mid (10-14 weeks PCW)
  "PCW10_BM_1_10x.h5",
  "PCW10_BM_2_10x.h5",
  "PCW11_BM_1_10x.h5",
  "PCW11_BM_2_10x.h5",
...
# Subset to only include samples in developmental_order (excludes mouse BM)
message("Current samples in data:")
current_samples <- unique(combined$orig.ident)
for(s in current_samples) message(paste("  -", s))

# Keep only samples that are in developmental_order
combined <- subset(combined, subset = orig.ident %in% developmental_order)

# Set factor order
combined$orig.ident <- factor(combined$orig.ident, levels = developmental_order)
message("Developmental order set successfully!")

# STEP 9: Generate unified scorecard
message("\n=== GENERATING UNIFIED SCORECARD ===")

pdf("Nascent_HSC_all_samples.pdf", width = 20, height = 10)

# Single DotPlot showing genes x sample source (now ordered by development)
p_unified <- DotPlot(combined, features = nascent_genes_available,
                     cols = c("grey90", "red3"),
                     group.by = "orig.ident") +
  coord_flip() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5, size = 8)) +
  ggtitle("Nascent HSC Signature Across All Samples (Developmental Order)")
print(p_unified)
